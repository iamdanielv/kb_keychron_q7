...
bool rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
    ...
    if (IS_LAYER_ON(KBCTL_LYR)) {
        ...
        const uint8_t accent_key_indexes[6] = {
            P_KI, // P for persistent color
            N_KI, // N for NKRO
            // arrows
            UP_KI, LEFT_KI, DOWN_KI, RIGHT_KI
        };
        set_keys_color((rgb_t){0xFF, 0xFF, 0xFF}, led_min, led_max, accent_key_indexes, 6 );
        ...
        // highlight right ctl as toggle Num Layer
        RGB_MATRIX_INDICATOR_SET_COLOR(RIGHT_CTL_KI, num_lyr_rgb.r, num_lyr_rgb.g, num_lyr_rgb.b);
    ...
    if (IS_LAYER_ON(MEDIA_LYR)) {
        const uint8_t media_keys[7] = {
            ...
            F7_KI, F8_KI, F9_KI, F10_KI, F11_KI, F12_KI, RIGHT_ALT_KI};
